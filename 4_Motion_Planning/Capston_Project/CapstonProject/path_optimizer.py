...
class PathOptimizer:
    ...
    def optimize_spiral(self, xf, yf, tf):
        ...
        p0 = [0.0, 0.0, sf_0]   # 1/3rd curvature, 2/3rd curvature, arc length
    ...
    def thetaf(self, a, b, c, d, s):
        ...
        #c = np.array(c)
        #d = np.array(d)
        #thetas = self._tf * len(a) + a * (s**4)/4 + b * (s**3)/3 + c * (s**2)/2 + d * s
    ...
    def sample_spiral(self, p):
        ...
        # x_points = self._xf + scipy.integrate.cumtrapz(np.cos(t_points), s_points, initial=None)
        # y_points = self._yf + scipy.integrate.cumtrapz(np.sin(t_points), s_points, initial=None)
