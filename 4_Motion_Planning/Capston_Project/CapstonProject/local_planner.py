...
class LocalPlanner:
    ...
    def get_goal_state_set(self, goal_index, goal_state, waypoints, ego_state):
        ...
        #theta = -ego_state[2]
        #goal_x = np.dot(-ego_state[0], np.array([[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]]))
        #goal_y = np.dot(-ego_state[1], np.array([[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]]))

        theta = -ego_state[2]
        goal_x = goal_state_local[0] * np.cos(theta) - goal_state_local[1] * np.sin(theta)
        goal_y = goal_state_local[0] * (np.sin(theta)) + goal_state_local[1] * np.cos(theta)
        ...
        for i in range(self._num_paths):
            ...
            offset = (i - self._num_paths // 2) * self._path_offset    # meaning?
