...
class BehaviouralPlanner:
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed):
        ...
        if self._state == FOLLOW_LANE:
            ...
            # ------------------------------------------------------------------
            if stop_sign_found:
                self._goal_state[2] = 0.0   # set the goal to zero speed
                self._state = DECELERATE_TO_STOP  # change state to decelerate to stop
                # ------------------------------------------------------------------
            pass
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        while wp_index < len(waypoints) - 1:
            arc_length += np.sqrt((waypoints[wp_index + 1][0] - waypoints[wp_index][0])**2 +
    ...
    def check_for_stop_signs(self, waypoints, closest_index, goal_index):
        ...
        for i in range(closest_index, goal_index):
            ...
            for stopsign_fence in self._stopsign_fences:
                wp_1   = np.array(waypoints[i][0:2])   # [x,y] position
                wp_2   = np.array(waypoints[i+1][0:2])  # [x,y] position
                s_1    = np.array(stopsign_fence[0:2])   # [x,y] for one point of stop lines
                s_2    = np.array(stopsign_fence[2:4])   # [x,y] for another point of stop lines

                v1     = np.subtract(wp_2, wp_1)  # [delta_x, delta_y]
                v2     = np.subtract(s_1, wp_2)   #
                sign_1 = np.sign(np.cross(v1, v2))  # direction of waypoints and stop lines
                v2     = np.subtract(s_2, wp_2)
                sign_2 = np.sign(np.cross(v1, v2))  # direction of waypoints and stop lines
    ...
    def check_for_lead_vehicle(self, ego_state, lead_car_position):
        ...
        if not self._follow_lead_vehicle:
            ...
            # vehicle lies within +/- 45 degrees of the ego vehicle's heading.   Q: why 45 degrees?
...
def get_closest_index(waypoints, ego_state):
    ...
    for i in range(len(waypoints)):
        ...
        y_i = waypoints[i][1]
        #v_i = waypoints[i, 2]
        distance = np.sqrt((x_i - x_ego)**2 + (y_i - y_ego)**2)
