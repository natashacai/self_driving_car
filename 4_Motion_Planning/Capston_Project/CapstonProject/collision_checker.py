...
class CollisionChecker:
    def __init__(self, circle_offsets, circle_radii, weight):
        self._circle_offsets = circle_offsets    # a list?
    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                # --------------------------------------------------------------
                circle_locations[:, 0] = path[0][j] + [i * int(np.cos(path[2][j])) for i in self._circle_offsets]
                circle_locations[:, 1] = path[1][j] + [i * int(np.sin(path[2][j])) for i in self._circle_offsets]

                # circle_locations[:, 1] = path[1][j] + self._circle_offsets * np.cos(path[2][j])
                # circle_locations[:, 1] = path[1][j] + self._circle_offsets * np.sin(path[2][j])
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                #score_x = np.linalg.norm(np.subtract(goal_state[0:2], paths[i][0:2][len(paths[i][0] - 1)]))  #deviation from the reference line
                #score_v = np.linalg.norm(np.subtract(goal_state[2], paths[i][2][len(paths[i][0] - 1)])) #deviation from the reference line
                
                #score_x = np.sqrt((goal_state[0] - paths[i][0][len(paths[i][0]) - 1]) ** 2 + (
                #            goal_state[1] - paths[i][1][len(paths[i][0]) - 1]) ** 2)
				#score_v = np.absolute((goal_state[2] - paths[i][0][len(paths[i][2]) #- 1]))
                #score = np.average(score_x) + score_v

                x_dists = np.array([paths[i][0][j] - goal_state[0] for j in range(len(paths[i][0]))])
                y_dists = np.array([paths[i][1][j] - goal_state[1] for j in range(len(paths[i][1]))])

                dist = np.sqrt(x_dists ** 2 + y_dists ** 2)
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                            # --------------------------------------------------
                            #score += self._weight * score[best_index]
                            #score += self._weight * paths[i][2][j]
                            score += self._weight * np.sqrt((paths[j][0][-1] - paths[i][0][-1])**2 + (paths[j][1][-1] - paths[i][1][-1])**2)
                            # --------------------------------------------------
                            pass
