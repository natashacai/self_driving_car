...
class Controller2D(object):
    def __init__(self, waypoints):
        ...
        self._nearest_waypoint_index = 0
    ...
    def update_desired_speed(self):
        # find nearest reference point relative to the current ego location
        min_idx       = 0
        min_dist      = float("inf")
        desired_speed = 0
        ...
        self._nearest_waypoint_index = min_idx
    ...
    def update_controls(self):
        ...
        nearest_waypoint_index = self._nearest_waypoint_index
        ...
        if self._start_control_loop:
            ...
            derivative_term = K_D * ((speed_error - self.vars.error_previous)/dt)
            ...
            if desired_acceleration > 0:
                ...
                if throttle_output - self.vars.throttle_previous > 0.05:
                    throttle_output = self.vars.throttle_previous + 0.05
            else:
                throttle_output = 0
            ...
            path_slope = (waypoints[nearest_waypoint_index][1] - waypoints[nearest_waypoint_index-1][1]) /\
                         (waypoints[nearest_waypoint_index][0] - waypoints[nearest_waypoint_index-1][0])
            c = waypoints[nearest_waypoint_index-1][1] - path_slope * waypoints[nearest_waypoint_index-1][0]
            b = 1.0
            a = -path_slope
            ...
            a = -path_slope
            b = 1.0
            c = (path_slope * waypoints[0][0]) - waypoints[0][1]

            # heading error
            path_yaw_rate = np.arctan2(waypoints[-1][1]-waypoints[0][1], waypoints[-1][0]-waypoints[0][0])
            heading_rate_difference = path_yaw_rate - yaw
            if heading_rate_difference > np.pi:
                heading_rate_difference -= self._2pi
            if heading_rate_difference < - np.pi:
                heading_rate_difference += self._2pi

            #######################################################################################
            yaw_path = np.arctan2(waypoints[-1][1] - waypoints[0][1], waypoints[-1][0] - waypoints[0][0])
            current_x_y = np.array([x, y])
            cross_track_error = np.min(np.sum((current_x_y - np.array(waypoints)[:, :2])**2, axis=1))

            #cross_track_error = (a * x + b * y + c) / np.sqrt(a ** 2 + b ** 2)  #not accurate enough


            ##############################################################################################
            k_e = 0.25
            delta_correction = yaw_path - np.arctan2(y - waypoints[0][1], x - waypoints[0][0])
            if delta_correction > np.pi:
                delta_correction -= self._2pi
            elif delta_correction < - np.pi:
                delta_correction += self._2pi
            ...
            # Change the steer output with the lateral controller.
            steer_output = heading_rate_difference + cross_track_term
            if steer_output >= 1.22:
                steer_output = 1.22
            elif steer_output <= -1.22:
                steer_output = -1.22
